...
El experimento de calcular el ARI promedio para cada característica por separado nos permitió tener una visión más detallada de qué tipo de características de Grambank parecen estar jugando un papel relevante para que el clustering sea similar al obtenido con BPE. Resulta interesante observar en la Tabla X que parece haber una predominancia de características relacionadas con la morfología en los primeros lugares del ranking de ARI promedio, mientras que las características tipológicas más relacionadas con la sintaxis y la morfosintaxis ocupan lugares menos prominentes en el ranking.

Si bien esto no constituye un continuum perfecto, en el que las características morfológicas aparecen primero, seguidas de las morfosintácticas y finalmente de las sintácticas, sí parece haber evidencia de que las características de Grambank que hacen que el clustering sea más parecido al obtenido con BPE muestran una tendencia hacia fenómenos morfológicos. Lo anterior hace sentido, pues esperaríamos que los agrupamientos obtenidos con BPE tengan mayor coincidencia con aquellos obtenidos a partir de características meramente morfológicas de Grambank. Recordemos que el algoritmo BPE no cruza el nivel de la palabra ortográfica, por lo que no captura directamente fenómenos sintácticos, sino patrones de símbolos adyacentes dentro de la palabra, relacionados con la estructura interna de esta, es decir, con fenómenos morfológicos.  

#underline[_Ximena: Considerar si pones la Tabla de Grambank en inglés sin traducir, _]

Aún más interesante resulta observar que, entre las características morfológicas mejor posicionadas, varias se relacionan con procesos productivos, es decir, patrones utilizados para codificar información gramatical, como el plural, así como otros procesos flexivos y derivativos. Por ejemplo, la característica GB047, “Is there a productive morphological pattern for deriving an agent noun from a verb?”, pregunta si una lengua utiliza un patrón morfológico productivo  mediante un morfema o afijo que pueda aplicarse a verbos para crear sustantivos agentivos. En náhuatl, por ejemplo, el verbo tlahtoa (‘hablar’) puede formar tlahtoa-ni (‘el que habla’) mediante el morfema agentivo -ni. Desde la perspectiva de BPE, _-ni_ puede aparecer como una subunidad recurrente al final de distintos sustantivos agentivos. Por lo tanto, un patrón morfológico productivo de este tipo puede generar regularidades formales (que se observan en la forma de las palabras) que un tokenizador BPE tiene la posibilidad de capturar a través de sus subwords.

Si bien existen fenómenos morfológicos de diversa naturaleza, los resultados sugieren que la representación de las lenguas obtenida a partir de BPE presenta una mayor compatibilidad con características morfológicas relacionadas con la productividad. Esto puede deberse al tipo de métricas que aplicamos a las subwords de BPE (productividad, idiosincrasia y c, freq). En particular, la medida de productividad captura regularidades o patrones recurrentes pues cuantifica cuántas palabras del corpus contienen una determinada subword y, por tanto, qué tan frecuentemente esta se combina o aparece como parte de distintas palabras. Desde un punto de vista lingüístico, cuando una regla morfológica es productiva genera estas regularidades de manera sistemática en distintas palabras, es posible que dichos patrones sean aprendidos y representados por BPE en sus subwords.

A medida que avanzamos en el ranking, las características se desplazan progresivamente de los procesos productivos de formación de palabras hacia los procesos morfosintácticos y de organización sintáctica, que parecen ser distantes de lo que codifica BPE. 
