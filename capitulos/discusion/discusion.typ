...
#underline[_Ximena:Sugiero iniciar directamente con esta subsección, y el párrafo que está arriba retomar una parte pero al final, cuando ya analziaste las distintas coincidencias_]

== La coincidencia con BPE

La coincidencia entre $X_"BPE"$ y $X_W$ que reportaron #cite(<ximena-bpe-2023>, form: "prose") se mantuvo cuando dejamos de depender de una sola configuración de agrupamiento y cuando medimos con otra métrica, el ARI, que toma en consideración cómo están organizados los grupos x#underline[_imena: Ser más claro en estas explicaciones, Silhouette conefficient también se basaba en la configuración de lso clusters_]. Notemos que los grupos en $X_0$ no mostraron un indicio a reproducir la organización de $X_W$, mientras que los grupos generados en $X_"BPE"$ obtuvieron resultados hasta el tripe respecto a esos valores de referencia. Esto es importante, porque nos da un indicio de una relación en como el espacio inducido por BPE organiza las lenguas respecto a un espacio definido por características morfológicas.

Además, esta coincidencia no solo está en una sola base de datos. Con Grambank, la coincidencia también superó a su referencia, y lo realizó en todos los puntos del barrido de características. Esto da más evidencia sobre la morfología que codifica $X_"BPE"$, pues Grambank codifica sus características de manera diferente a WALS. Incluso el techo de los resultados de ARI en Grambank es más alto que el de WALS, pero hay que hacer énfasis en un pequeño paréntesis: que se realizó con menos lenguas estos experimentos, lo cual tiene cierta interferencia con la comparabilidad de los resultados.
...
Queda entonces descartar que esos picos no sean resultado de grupos muy grandes. Un ARI alto puede salir de un agrupamiento arbitrario que coincida por casualidad debido al tamaño de los grupos, por ejemplo al medir particiones con grupos de uno o dos elementos. Por lo tanto, analizamos las agrupaciones que lo producen en configuraciones en el techo de los resultados (no necesariamente el valor máximo). Las figuras @ejemplo-espacios-clusters-wals-bpe, @ejemplo-espacios-clusters-grambank-bpe y @ejemplo-espacios-clusters-grambankANDwals-bpe muestran, para cada comparación, una configuración de ARI alto sobre $X_"BPE"$ junto a una configuración de ARI alto sobre $X_0$, de modo que el contraste es entre los mejores casos de cada espacio y no entre uno bueno y uno cualquiera.
...
#underline[_Ximena: Sugiero que este párrafo que sigue, lo sustituyas/adapates con lo que te puse arriba_]
