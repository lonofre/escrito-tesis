...
//La pregunta concreta que esta tesis se propone responder es, entonces: ¿el agrupamiento que induce el espacio de BPE coincide con el que induce cada base lingüística por encima de lo que cabría esperar al azar, y se mantiene esa coincidencia al variar la base, la dimensionalidad y la semilla?
...
//Partiendo del trabajo previo, ¿cómo cambian las observaciones cuando se incorpora una metodología más robusta para comparar si las lenguas caracterizadas con BPE tienen similitud a als caracterizaciones obtenidas con WALS? la comparación de agrupamientos, considerando diferentes configuraciones de clustering obtenidas a partir de semillas aleatorias e incorporando métricas adicionales para evaluar la similitud entre agrupamientos?
