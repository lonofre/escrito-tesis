...
Para calcular la similitud entre un espacio $X$ y un espacio $Y$, se utilizó el ARI sobre los agrupamientos de cada espacio, obtenidos mediante k-medias++ con $k = 4$, siguiendo el experimento original de #cite(<ximena-bpe-2023>, form: "prose"). Con un $k$ menor el agrupamiento resultaría trivial. Este algoritmo depende de una semilla de inicialización, por lo que los resultados pueden variar entre ejecuciones. Para dar cuenta de esta variabilidad, se evaluaron todos los pares ordenados $(s_1, s_2)$ con $s_1 != s_2$ tomados de ${0, 1, dots, 99}$, lo que generó un total de $9,900$ valores de ARI para cada par de espacios $X, Y$. Se eligieron pares ordenados ---y no combinaciones--- porque $s_1$ inicializa el agrupamiento de $X$ y $s_2$ el de $Y$ de forma independiente, por lo que invertir el par produce agrupamientos distintos y, en consecuencia, otro valor de ARI.
