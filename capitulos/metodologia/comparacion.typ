...
Se compararon $X_"BPE"$ y $X_G in RR^(|L_G| times d_G)$, corriendo el experimento para cada valor de $d_G$ en el conjunto $D$. Esto produce una distribución de $9,900$ valores de ARI por cada $d_G$ evaluado.
...
Se construyó el espacio combinado $X_(W+G) = [X_W | X_G] in RR^(|L_G| times (d_W + d_G))$ por concatenación horizontal, con $|L_G| = #lenguas_grambank$ lenguas cubiertas por Grambank, $d_W = 15$ fijo (@wals-features) y $d_G$ variable sobre $D$. Se comparó $X_"BPE"$ con $X_(W+G)$ para cada valor de $d_G$, produciendo una distribución de $9,900$ valores de ARI por cada $d_G$ evaluado.
...
Se compararon $X_G in RR^(|L_G| times d_G)$ y $X_W$ restringido al mismo conjunto $L_G$ de lenguas, corriendo el experimento para cada valor de $d_G$ en $D$. Esto produce una distribución de $9,900$ valores de ARI por cada $d_G$ evaluado.
...
Se compararon $X_G in RR^(|L_G| times d_G)$ con $X_"l2v"$ en sus dos variantes, `syntax_wals` y `syntax_knn`, corriendo el experimento para cada valor de $d_G$ en $D$. Esto produce una distribución de $9,900$ valores de ARI por cada $d_G$ evaluado y por cada variante de lang2vec.
