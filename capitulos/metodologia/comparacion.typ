...
*/
Para crear los agrupamientos en cada espacio, usamos el algoritmo de k-medias, pues es un algoritmo particional que nos permitió dividir los espacios en $k$ grupos. Además, a diferencia de otros algoritmos de  comunes de agrupamiento, tiene una complejidad lineal en tiempo. En particular, empleamos la variante k-medias++#kmeans_footnote, que ofrece una mejor inicialización de los centroides. En cuanto al número de grupos, elegimos $k = 4$, siguiendo la metodología de #cite(<ximena-bpe-2023>, form: "prose"), donde eligen este número debido a que es valor lo suficientemente grande para distinguir bien los grupos (a diferencia de 2 o 3). También coincide con posibles clases de lenguas identificables de acuerdo con sus características morfológicas; por ejemplo, un grupo con mayor tendencia analítica, otro con tendencia fusional, sintética, polisintética, etc., pero no tan grande como para dificultar la interpretación de las relaciones entre ellos. Además, para no depender de una sola semilla en la inicialización de los centroides, utilizamos las semillas del conjunto ${0, 1, 2, dots, 99}$ en cada experimento, lo cual nos dio 100 agrupamientos distintos por espacio.
...
Para ello, desglosamos la comparación entre $X_"BPE"$ y $X_G$ característica por característica. Es decir, iteramos sobre cada característica de Grambank y, para cada una, realizamos un agrupamiento de las lenguas utilizando únicamente los valores de dicha característica. Posteriormente, comparamos este agrupamiento con el obtenido a partir de las representaciones BPE. Para cada característica, repetimos el procedimiento con la misma configuración de semillas que en los experimentos anteriores (100 semillas por espacio, es decir, 10,000 pares) y calculamos el ARI en cada iteración. Finalmente, reportamos el ARI promedio de cada característica de Grambank, con el objetivo de identificar aquellas características que muestran una mayor similitud entre sus agrupamientos y los obtenidos en el espacio BPE. Cabe destacar que no analizamos todas las características de Grambank, sino un subconjunto del barrido de características, que fijamos a partir de lo observado en los experimentos de la sección anterior.
