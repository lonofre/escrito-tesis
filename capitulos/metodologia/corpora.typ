...
El PBC reúne 994 traducciones de la Biblia distribuidas en 837 lenguas distintas según el estándar ISO 639-3, cobertura que incluye las 47 lenguas analizadas. Cada traducción está alineada a nivel de versículo mediante identificadores estandarizados, normalizada en Unicode y tokenizada a nivel de palabra, propiedades que la hacen adecuada para el procesamiento computacional (véase @ejemplo-PBC). Al tratarse de un corpus paralelo, todas las lenguas comparten esencialmente el mismo contenido textual; esto hace comparables los modelos BPE entre lenguas, pues las diferencias que el algoritmo capture son atribuibles a la estructura de cada lengua y no a diferencias de dominio o tema.
...
Por otro lado, decidir cuántas y cuáles características de Grambank utilizar fue una decisión que tuvimos que resolver nosotros mismos, pues no contábamos con un criterio claro para hacerlo. En primer lugar, esto se debe a que el número de lenguas con un valor definido varía de una característica a otra. Los criterios de GB020 muestran de dónde sale esa variación: el 1 se asigna cuando la fuente documenta un artículo definido; el 0 cuando una fuente por lo demás exhaustiva no registra ninguno; y el "?" cuando el análisis no alcanza para decidirlo. A esto se suma que una característica puede no tener ningún valor registrado para una lengua. Tanto el "?" como el valor ausente los tratamos como valores faltantes. Así, solo 6 características tienen valor definido en las 38 lenguas que analizamos, mientras que la peor documentada apenas llega a 14. Por eso, elegir una característica cualquiera no garantiza poder comparar con ella a todas las lenguas, y pedir que las cubra todas nos deja con muy pocas. En segundo lugar, a diferencia de WALS, Grambank no agrupa sus características por área lingüística, lo cual nos habría proporcionado un criterio de selección sin necesidad de contar con conocimiento lingüístico especializado.
...
De los conjuntos de características sintácticas que ofrece lang2vec, elegimos `syntax_knn`, que imputa los valores faltantes mediante $k$ vecinos más cercanos, ya que es el único que cubre las 47 lenguas del estudio sin valores vacíos y predice esos valores con una exactitud alrededor del 92% en validación cruzada @littell2017uriel. Los demás conjuntos se distinguen por sus fuentes o por cómo tratan los valores faltantes: `syntax_wals`, `syntax_sswl` y `syntax_ethnologue` provienen de una sola base cada uno, y `syntax_avg` los promedia. En estos conjuntos, mientras más lenguas y características abarcan, más valores vacíos tienen. Un ejemplo es `syntax_wals`, que es el más amplio, solo llena el 44% de sus valores, e incluso promediar no ayuda mucho, pues `syntax_avg` alcanza apenas el 34% de cobertura.

Por último, en contraste con Grambank, lang2vec usa el ISO 639-3 para la identificación de lenguas. Por ende, se usó la misma relación con los Glottocodes para relacionar a lang2vec con Grambank.