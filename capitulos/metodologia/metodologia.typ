...
Nuestra hipótesis dice que, si las lenguas se agrupan de forma parecida en el espacio de BPE y en los espacios construidos a partir de las bases de datos lingüísticas, entonces BPE captura parte de la estructura que describen los lingüistas. Para ponerla a prueba, representamos cada fuente de la misma manera: como un espacio vectorial donde cada renglón es una lengua y cada columna una característica, sin importar si esa característica viene de contar subpalabras o de la descripción de un especialista. Sobre esos espacios agrupamos a las lenguas y medimos qué tanto coinciden los agrupamientos.

El capítulo se divide en tres partes. Primero describimos de dónde vienen los datos: el corpus paralelo con el que generamos los modelos de BPE y las bases de datos lingüísticas (WALS, Grambank y lang2vec) de donde tomamos las características descritas por especialistas. Después explicamos cómo convertimos cada fuente en un espacio vectorial, pues cada base cubre un conjunto distinto de lenguas y codifica sus características de forma distinta. Al final planteamos cómo medimos el parecido entre los espacios y qué experimentos hicimos con ellos.
