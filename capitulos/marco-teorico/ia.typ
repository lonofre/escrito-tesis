...
Como se observa en @diagrama-transformers, un Transformer recibe como entrada una secuencia de tokens (unidades en las que se divide el texto), sigue una serie de operaciones matriciales dentro de los bloques, y da como salida una distribución de probabilidad sobre el siguiente token. La estructura de estas operaciones está definida por el diseño de la arquitectura, mientras que sus parámetros, es decir, los valores concretos que usan, se aprenden a partir de los tokens observados durante el entrenamiento. El objetivo no cambió respecto a los n-gramas, porque el modelo sigue estimando la probabilidad del siguiente elemento dado el contexto previo y con esa estimación genera la secuencia un elemento a la vez @Mijangos-Ximena-2024. Lo que cambió es la unidad sobre la que se estima, que ya no es la palabra sino el token.

Ese token es el producto de la tokenización, que puede verse como el proceso de segmentar las cadenas de texto en unidades más fundamentales. Estas unidades son convertidas después a representaciones numéricas (vectores/embeddings) que sirven para entrenar una red neuronal artificial. Un modelo del lenguaje no aprende cuáles son las unidades más apropiadas en las que se debe segmentar una cadena en lenguaje natural. Más bien, la tokenización es un proceso aparte que tiene mucho impacto en el desempeño del sistema final.

Por ende, cabe preguntarse qué influencia tienen los tokens sobre estos modelos. De cómo se realice esa segmentación, y en particular de qué unidades produzca, depende la representación inicial que el modelo recibe como entrada, y con ella, el punto de partida sobre el que se construye todo lo demás.