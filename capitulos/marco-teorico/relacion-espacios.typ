...
El agrupamiento (_clustering_) es la tarea de dividir un conjunto de datos en grupos (clusters), de modo que los datos dentro de cada grupo sean más parecidos entre sí que con los de otros grupos. Para determinar esta similitud se toman en cuenta los atributos de los datos, comúnmente a través de medidas de distancia @han2012data.
...
#cite(<ximena-bpe-2023>, form: "prose") usaron k-medias para generar agrupamientos en los experimentos que hicieron con la caracterización de BPE y la base de datos lingüística WALS. Primero agruparon las lenguas en el espacio de WALS, construido a partir de las características de tipología morfológica que seleccionaron, bajo la premisa de que lenguas tipológicamente similares deberían quedar cerca unas de otras en ese espacio. Después, compararon esos agrupamientos pero usando los vectores de las lenguas de BPE, para ver qué tanto coincidían esos agrupamientos en ese espacio. La forma de encontrar esta coincidencia fue usar el Silhouette, un método de validación para el análisis de grupos.
...
El Índice Rand (Rand Index, RI) @rand mide la similitud entre dos particiones de un conjunto. Para esto, revisa cada par de objetos y determina si ambas particiones los colocan de forma consistente, es decir, si los ubican en el mismo grupo en las dos particiones o en grupos distintos en las dos particiones. La proporción de pares en los que existe esta coincidencia define el valor del índice, que toma valores entre 0 (ningún acuerdo) y 1 (acuerdo completo).

Sin embargo, el Índice Rand presenta una limitación importante. Dos particiones generadas al azar pueden alcanzar valores altos, ya que el índice no descuenta el grado de coincidencia esperado por azar @Hubert1985. Esto dificulta interpretar valores intermedios y motivó la formulación de una versión corregida, conocida como el Índice Rand Ajustado.
