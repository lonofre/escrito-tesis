...
La tipología lingüística estudia las semejanzas y diferencias estructurales que existen entre las distintas lenguas del mundo @velupillai2012. La tipología morfológica realiza este estudio a nivel de las diferencias de los procesos y estructuras que intervienen en la formación de palabras en una lengua. Por ejemplo, si pensamos en términos de morfemas, las unidades más pequeñas de una palabra que poseen algún tipo de significado, ya sea gramatical o léxico (tri-sílaba, come-mos, niñ-a-s), podría ser de interés conocer cómo funcionan estos mecanismos en las diferentes lenguas que se hablan en el mundo para poder clasificarlas. 

Hay lenguas que suelen tener una gran cantidad de morfemas por palabra, mientras que en otras los procesos morfológicos son más limitados y hay pocos morfemas por palabra o, incluso, las palabras carecen de procesos morfológicos internos. En esta escala de clasificación tipológica podemos encontrar lenguas _analíticas_, con pocos morfemas formando una palabra; aquí encontraríamos lenguas como el chino o incluso el inglés, con una morfología relativamente reducida. En un punto intermedio se encuentran las lenguas _sintéticas_, que suelen sintetizar o codificar en una sola palabra gran cantidad de información mediante el uso de diferentes morfemas (como ocurre en el alemán o el turco). Finalmente, encontramos las lenguas _polisintéticas_, donde el uso extensivo de morfemas dentro de una palabra les permite codificar en una sola palabra lo que en otras lenguas más analíticas requiere varias palabras (aquí estarían lenguas como el kalaallisut, el náhuatl o el quechua, entre otras) @sapir1921language @velupillai2012.

Además del grado de _síntesis_, otra dimensión clásica para clasificar la morfología de las lenguas es el grado de _fusión_ de los morfemas, descrito por @sapir1921language. Podemos encontrar lenguas _fusionales_ (como el latín o el español), donde un solo morfema fusiona diferentes funciones gramaticales. Por ejemplo, en español, el morfema -mos en la terminación de un verbo es una flexión que codifica simultáneamente información sobre la persona, el tiempo, el aspecto y el modo. También hay lenguas _aglutinantes_, que van concatenando morfemas, donde cada uno codifica una función gramatical distinta (como el turco o el náhuatl). Finalmente, encontramos las lenguas _aislantes_, en las que cada morfema que codifica una función gramatical o información léxica constituye una palabra independiente; es decir, los procesos morfológicos son mínimos, ya que prácticamente no existen flexiones ni derivaciones dentro de las palabras.
...
La *frecuencia acumulada* de una subpalabra $s$ es la frecuencia de $s$ en el corpus $W$. Esta medida es independiente de si esta subpalabra aparece contenida en una palabra como si aparece de manera independiente.

$ "c.freq"(s) = sum_(w in W_s) "freq"(w) $

La *idiosincrasia* hace referencia a esas subpalabras que son muy frecuentes pero pocas palabras las contienen. Por ello, se calcula dividiendo la frecuencia acumulada de una subpalabra $s$ en un corpus $W$, $"c.freq"(s)$, entre el número de palabras en las que aparece, $"productividad"(s)$:
